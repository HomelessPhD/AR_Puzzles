...
def t_job(t_id, T_FOUND_ev, left, right, answers_list, B_A_ev):
    ...
    for Ib in range(0, (right - left) // BATCH_SIZE + 1):
        ...
        process = subprocess.Popen(["./pzl12_GOscript"], stdin=subprocess.PIPE, stdout=subprocess.PIPE, text=True)
...
import itertools
idxs_list = list(itertools.permutations([0,1,2,3]))

for idxs in idxs_list:
    for k1 in keys_list[ idxs[0] ]:
        for k2 in keys_list[ idxs[1] ]:
            for k3 in keys_list[ idxs[2] ]:
                for k4 in keys_list[ idxs[3] ]:
                    answer = ''+k1+k2+k3+k4
                    if len(answer) == 58:
