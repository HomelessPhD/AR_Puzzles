...
def t_job(t_id, T_FOUND_ev, left, right, answers_list, B_A_ev):
    ...
    for Ib in range(0, (right - left) // BATCH_SIZE + 1):
        ...
        B_A_ev.clear()
        with open('pzl10_bruttedAnswers.txt', 'a') as f:
...
brutted_answers = set()
if os.path.exists('pzl10_bruttedAnswers.txt'):
    with open('pzl10_bruttedAnswers.txt') as f:
